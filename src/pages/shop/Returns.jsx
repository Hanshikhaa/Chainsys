...
import { Package, Clock, CheckCircle2, AlertCircle, MapPin, RotateCcw, X, ShieldAlert } from 'lucide-react';
...
const Returns = () => {
    ...
    // Only show orders that are eligible for return or have already been requested/returned
    const eligibleOrders = orders.filter(
        o => o.status === 'Received' || o.status === 'Return Requested' || o.status === 'Returned'
    );
    ...
    const getStatusStyle = (status) => {
        switch (status) {
            ...
            case 'Returned': return { color: '#0ea5e9', bg: '#f0f9ff', icon: <ShieldAlert size={16} /> };
    ...
    return (
        <div className="container section-padding">
            <h1 style={{ marginBottom: '2.5rem' }}>Returns & Replacements</h1>
            {eligibleOrders.length === 0 ? (
                <div className="card" style={{ padding: '4rem', textAlign: 'center' }}><RotateCcw size={48} color="var(--text-muted)" style={{ opacity: 0.3 }} /><h3>No eligible orders found for returns.</h3></div>
            ) : (
                <div style={{ display: 'grid', gap: '1.5rem' }}>
                    {[...eligibleOrders].reverse().map(order => {
                                ...
                                <div className="flex-between">
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.5rem' }}>
                                        <div style={{ backgroundColor: style.bg, color: style.color, padding: '0.4rem 0.8rem', borderRadius: '20px', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.4rem', fontWeight: '700' }}>
                                            <span style={{ fontSize: '0.7rem', opacity: 0.7, textTransform: 'uppercase' }}>Status:</span> {style.icon} {order.status}
                                        </div>
                                        {order.status === 'Received' && (
                                            <button onClick={() => setReturnModalOpen(order.id)} className="btn btn-outline" style={{ padding: '0.3rem 0.6rem', fontSize: '0.8rem' }}>
                                                <RotateCcw size={14} style={{ marginRight: '0.2rem' }} /> Initiate Return
                                            </button>
                                        )}
                                    </div>
                                </div>
